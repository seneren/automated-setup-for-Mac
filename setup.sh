...
configure_git() {
  echo "Setting up Git..."
  git config --global user.name "Eren Şen"
  git config --global user.email "eren.sen1988@gmail.com"
  git config --global core.editor "code --wait"
  git config --global push.default upstream

  echo "Generating SSH keys..."
  mkdir -p ~/.ssh
  if ! ssh-keygen -t ed25519 -C "eren.sen1988@gmail.com" -f ~/.ssh/id_ed25519 -N ""; then
  ...
  if [[ "$SETUP_PAT" == "y" ]]; then
    ...
    echo "https://eren.sen1988:$GITHUB_PAT@github.com" | git credential-osxkeychain store
